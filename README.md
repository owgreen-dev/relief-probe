...
- **Detectors:** `naics_cohort_outlier` (robust cohort z-score, BH-FDR) · `payroll_cap_exceedance` (per-employee program ceiling) · `duplicate_address_ring` (the independent link-analysis signal — see below).
- **Independent corroboration (`duplicate_address_ring`):** a co-location / link-analysis detector that groups loans by a normalized building-level address key and flags addresses shared by ≥3 *distinct* borrowers, scored monotonically in ring size and total ring dollars. It answers the fair critique that the two $/job detectors are "two views of the same dollars-per-job ratio": this signal is orthogonal — it fires on rings whose per-job economics are perfectly normal, so when it corroborates a $/job flag the agreement is across genuinely independent evidence (a synthetic test proves this orthogonality; real-data overlap/lift is a documented manual post-loop step). Honest false-positive modes: legitimate shared office buildings, strip malls, co-working spaces, and registered-agent / commercial-filing addresses all collapse many unrelated borrowers to one key, so a ring is a review lead, not proof of a scheme.
