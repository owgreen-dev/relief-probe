...
scripts/            real-data validation harnesses (validate_*.py) + autonomous-dev (Ralph) tooling
plans/              the autonomous-dev (Ralph TDD) audit trail — PRD + progress log per build loop
```
...
```bash
uv run pytest                                       # ~205 offline tests
uv run relief-probe ingest --slice 150k_plus        # ~1M big-dollar loans (~430 MB), local DuckDB
uv run relief-probe score                           # run detectors → ranked leads
uv run relief-probe fetch-labels && uv run relief-probe resolve-labels   # DOJ enforcement → fraud_cases labels
uv run relief-probe benchmark                       # forward PU lift@k + baselines + bootstrap CIs
...
uv run --extra viz --extra vision --extra embeddings-lite \
    streamlit run app/dashboard.py                  # the dashboard (4 tabs)
```

More commands (LLM triage, KYB enrichment, MCP server, the validation harnesses) are in [docs/NEXT_STEPS.md](docs/NEXT_STEPS.md).
