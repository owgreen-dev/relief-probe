...
## The easy on-ramp: a Ralph loop

The fastest way to add a detector *with* this discipline is a bounded autonomous TDD loop —
see [`plans/`](plans/) for how the existing detectors (Loops 1–5) were built: a `prd.json`
feature spec, a `progress.md` log, and `guardrails.md` (the accumulated constraints). Each loop
implements one feature per iteration and only commits when `pytest` + `ruff` pass.
