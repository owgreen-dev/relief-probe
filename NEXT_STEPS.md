...
`duplicate_address_ring` (shared-address / link-analysis ring signal) added in **H6**.
...
- **H6 — One genuinely independent detector** ✅ Done (code + synthetic independence
  ...
  Orthogonality is proven on synthetic data (a ring with perfectly-normal $/job fires
  the ring detector while `naics_cohort_outlier` and `payroll_cap_exceedance` stay
  silent — `tests/test_ring_detector.py`); a `detector_overlap` helper
  (`benchmark/core.py`) computes Jaccard/intersection between two detectors' flagged
  sets. False-positive modes (shared office buildings, strip malls, registered-agent
  addresses) are documented in the README and RESPONSIBLE_USE.md.
  **MANUAL post-loop step (real-warehouse numbers, per the no-invented-numbers rule):**
  on the full warehouse run `relief-probe score` then `relief-probe benchmark` and
  record the real independence/overlap (`detector_overlap` Jaccard of
  `duplicate_address_ring` vs each $/job detector) and the lift@k change from adding the
  third detector. Those numbers are generated by a human on real data, not in the loop.
...
- **Done this session:** H6 — the genuinely independent `duplicate_address_ring`
  detector (code + synthetic orthogonality test), so corroboration is no longer two
  views of the same $/job ratio. Real-data overlap/lift is a documented manual
  post-loop step.
